...
import BookWriter from "xltpl/dist/es5/exceljs/xltpl.browser";
...
class App extends Component {
  ...
  handleXltplExportClick = (e) => {
    this.xltplExport();
  };

  xltplExport = () => {
    let tplFilename = "./tpl.xlsx";
    axios.get(tplFilename, { responseType: "arraybuffer" }).then((respond) => {
      let writer = new BookWriter();
      let p = writer.load(respond.data);
      let employees = this.employees;
      p.then(async function () {
        let payload = {
          ps: employees,
          header: "Employees of A Big Company",
          footer: "Nothing",
          sheetName: "employees"
        };
        writer.renderSheet(payload);
        payload["tpl_name"] = "employee";
        payload["date"] = new Date();
        for (let employee of employees) {
          payload["sheetName"] = employee.FirstName + " " + employee.LastName;
          payload["p"] = employee;
          writer.renderSheet(payload);
        }
  ...
  render() {
    return (
      <div>
        ...
        <Button
          text="export-With-Exceljs"
          type="danger"
          onClick={this.handleExportClick}
        />
        <Button
          text="export-With-Xltpl"
          type="danger"
          onClick={this.handleXltplExportClick}
        />
      </div>
    );
  }
